...
module.exports = async ({getNamedAccounts, deployments}) => {
  ...
  if((await TokenLock.lockedAmounts(TimeLock.address)).eq(0)) {
    ...
    await (await RandomizerToken.transfer(config.RNDD_TREASURY_GNOSIS_SAFE, oneToken.mul(config.LOCKED_BURNABLE_TOKENS))).wait();
    ...
    // await (await RandomizerToken.transfer(config.RNDD_TREASURY_GNOSIS_SAFE, oneToken.mul(config.TOTAL_DAILY_DRAW_TOKENS))).wait();
    ...
    await (await RandomizerToken.approve(TokenLock.address, config.TEAM_TREASURY_GNOSIS_SAFE)).wait();
    ...
    // const totalRNDDTokens = oneToken.mul(config.TOTAL_DAILY_DRAW_TOKENS);
    // await (await RandomizerToken.transfer(config.RNDD_TREASURY_GNOSIS_SAFE, totalRNDDTokens)).wait();
...
module.exports.dependencies = ['RandomizerToken', "RandomizerMetaGamePass", "RandomizerDailyDraw", 'TimeLock', 'TokenLock'];
